...
func (r *pngReader) GetImageWidth() int {
	return r.width
}

func (r *pngReader) GetImageHeight() int {
	return r.height
}

func (r *pngReader) GetBitDepth() byte {
	return r.bitDepth
}

func (r *pngReader) GetColorType() byte {
	return r.colorType
}

func (r *pngReader) GetCompressionMethod() byte {
	return r.compressionMethod
}

func (r *pngReader) GetFilterMethod() byte {
	return r.filterMethod
}

func (r *pngReader) GetInterlaceMethod() byte {
	return r.interlaceMethod
}

func (r *pngReader) GetImageData() []byte {
	return r.imgData
}
		...
		case "IDAT":
			...
			if data, err := r.GetBytes(chunkLen + 4); err != nil {
				...
				case "00": // - no compression
					...
					//
					if lenBytes, err := cr.GetBytes(4); err != nil {
						return nil, err
					} else {
						l := bytesToInt(lenBytes[:2])
						d, err := cr.GetBytes(l)
						if err != nil {
							return nil, err
						}
