...
func main() {
	...
	width := 0                   //  4 bytes
	height := 0                  //  4 bytes
	bitDepth := byte(0)          //  1 byte
	colorType := byte(0)         //  1 byte
	compressionMethod := byte(0) //  1 byte
	filterMethod := byte(0)      //  1 byte
	interlaceMethod := byte(0)   //  1 byte
	...
	for {
		...
		*/
		...
		case "IHDR":
			...
			*/
			if data, err := r.GetBytes(chunkLen + 4); err != nil {
				...
				width = int(data[3]) + int(uint32(data[2])<<8) + int(uint32(data[1])<<16) + int(uint32(data[0])<<24)
				height = int(data[7]) + int(uint32(data[6])<<8) + int(uint32(data[5])<<16) + int(uint32(data[4])<<24)
				bitDepth = data[8]
				colorType = data[9]
				...
				//                       followed by an alpha sample.
				log("compression method:", compressionMethod)
				...
				log("filter method:", filterMethod)
				...
				log("interlace method:", interlaceMethod)
		...
		case "IEND":
			log("it's IEND")
			for _, d := range getCanonicalHuffmanCodingMap(map[int]int{5: 32}) {
				for i := 0; i < 32; i++ {
					log(i, ":", d[i])
				}
			}
		...
		case "IDAT":
			...
			if data, err := r.GetBytes(chunkLen + 4); err != nil {
				...
				cr := bitreader.GetNewSliceBitReader(data)
				...
				case "10": // - compressed with dynamic Huffman codes
					...
					for i := 0; i < len(lengthCodes); i += 3 {
						...
						log(lenDictionary[j], ":", lengthCodes[i], lengthCodes[i+1], lengthCodes[i+2], "=", l, "OR", int(lengthCodes[i+2]+lengthCodes[i+1]*2+lengthCodes[i]*2*2))
					...
					for i := 0; i < hlit; i++ {
						...
						//log(i, ":", uncompressedLitTreeData[i])
					...
					dat, err := decodeLZ77(cr, litTree, distTree)
					if err != nil {
						panic(err)
					}
					log("decoded LZ77", len(dat))
...
func getDistanceLength(cr bitreader.BitReader, val int, distTree map[string]int) (int, int, error) {
	...
	case 264:
		...
		return d, l, nil
	case 265:
		l, err := __getValWithExtraBits(cr, 11, 1)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 266:
		l, err := __getValWithExtraBits(cr, 13, 1)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 267:
		l, err := __getValWithExtraBits(cr, 15, 1)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 268:
		l, err := __getValWithExtraBits(cr, 17, 1)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 269:
		l, err := __getValWithExtraBits(cr, 19, 2)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 270:
		l, err := __getValWithExtraBits(cr, 23, 2)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 271:
		l, err := __getValWithExtraBits(cr, 27, 2)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 272:
		l, err := __getValWithExtraBits(cr, 31, 2)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 273:
		l, err := __getValWithExtraBits(cr, 35, 3)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 274:
		l, err := __getValWithExtraBits(cr, 43, 3)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 275:
		l, err := __getValWithExtraBits(cr, 51, 3)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 276:
		l, err := __getValWithExtraBits(cr, 59, 3)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 277:
		l, err := __getValWithExtraBits(cr, 67, 4)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 278:
		l, err := __getValWithExtraBits(cr, 83, 4)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 279:
		l, err := __getValWithExtraBits(cr, 99, 4)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 280:
		l, err := __getValWithExtraBits(cr, 115, 4)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 281:
		l, err := __getValWithExtraBits(cr, 131, 5)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 282:
		l, err := __getValWithExtraBits(cr, 163, 5)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 283:
		l, err := __getValWithExtraBits(cr, 195, 5)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
		...
		return d, l, nil
	case 284:
		l, err := __getValWithExtraBits(cr, 227, 5)
		if err != nil {
			return 0, 0, err
		}
		d, err := getDistance(cr, distTree)
	...
	for {
		...
		if val, exists := tree[key]; !exists {
			...
			case 3:
				return 4, nil
			case 4:
				return __getValWithExtraBits(cr, 5, 1)
			case 5:
				return __getValWithExtraBits(cr, 7, 1)
			case 6:
				return __getValWithExtraBits(cr, 9, 2)
			case 7:
				return __getValWithExtraBits(cr, 13, 2)
			case 8:
				return __getValWithExtraBits(cr, 17, 3)
			case 9:
				return __getValWithExtraBits(cr, 25, 3)
			case 10:
				return __getValWithExtraBits(cr, 33, 4)
			case 11:
				return __getValWithExtraBits(cr, 49, 4)
			case 12:
				return __getValWithExtraBits(cr, 65, 5)
			case 13:
				return __getValWithExtraBits(cr, 97, 5)
			case 14:
				return __getValWithExtraBits(cr, 129, 6)
			case 15:
				return __getValWithExtraBits(cr, 193, 6)
			case 16:
				return __getValWithExtraBits(cr, 257, 7)
			case 17:
				return __getValWithExtraBits(cr, 385, 7)
			case 18:
				return __getValWithExtraBits(cr, 513, 8)
			case 19:
				return __getValWithExtraBits(cr, 769, 8)
			case 20:
				return __getValWithExtraBits(cr, 1025, 9)
			case 21:
				return __getValWithExtraBits(cr, 1537, 9)
			case 22:
				return __getValWithExtraBits(cr, 2049, 10)
			case 23:
				return __getValWithExtraBits(cr, 3073, 10)
			case 24:
				return __getValWithExtraBits(cr, 4097, 11)
			case 25:
				return __getValWithExtraBits(cr, 6145, 11)
			case 26:
				return __getValWithExtraBits(cr, 8193, 12)
			case 27:
				return __getValWithExtraBits(cr, 12289, 12)
			case 28:
				return __getValWithExtraBits(cr, 16385, 13)
			case 29:
				return __getValWithExtraBits(cr, 24577, 13)
			}
		}
	}
...
func __getValWithExtraBits(cr bitreader.BitReader, startVal int, numExtraBits int) (int, error) {
	...
	str := ""
	for _, bit := range bitsE {
		str += fmt.Sprint(bit)
	}
	log("read next ", numExtraBits, " bits, they are", str)
	log("the real value is", startVal, "+", cr.ToInt(bitsE), "or", cr.ToInt(reverseSliceByte(bitsE)))
	...
	for {
		...
		if val, exists := canonicalHuffmanCodingMapForLengthsTree[key]; !exists {
			...
			if byte(val) == 16 {
				...
				prevVal := byte(0)
				if n > 0 {
					prevVal = byte(uncompressedTreeData[n-1])
				}
...
func getCanonicalHuffmanCodingMapForLengthsTree(lenHCodes map[int]int, lenDictionaryMap map[int]int) map[string]int {
	...
	result := make(map[string]int, 0)
	for l := 0; l <= 18; l++ {
		//for _, l := range []int{16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15} {
...
func getCanonicalHuffmanCodingMap(dictionary map[int]int) map[int][]string {
	...
	for _, k := range keysSorted {
		...
		for j := 1; j < dictionary[k]; j++ {
			number, _ := strconv.ParseUint(canonicalHuffmanCodingMap[k][j-1], 2, 32)
			n := int(number)
...
func log(msgs ...interface{}) {
	...
	baseDir := "/tmp/ramdisk/go/src/assa.com/put.pixel"
